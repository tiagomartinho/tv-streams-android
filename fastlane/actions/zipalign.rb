    ...
    class ZipalignAction < Action
      def self.run(params)
       ...
       error_callback = proc do |error|
         ...
         aligncmd = ["$ANDROID_HOME/build-tools/28.0.2/zipalign -v -f 4", new_name , " ", params[:apk_path] ]
         Fastlane::Actions.sh(aligncmd, log: true)

         return
        end

       zipalign = Fastlane::Actions.sh("$ANDROID_HOME/build-tools/28.0.2/zipalign -c -v 4 #{params[:apk_path]}", log: false , error_callback: error_callback)
